...
const BarGraph = ({ data, label }) => (
  <div className="w-full h-64 my-4 flex justify-center items-center">
    <div className="w-3/4">
      <h3 className="text-center text-lg font-semibold mb-2 text-white">{label}</h3>
...
const RecordCard = ({ record }) => (
  <div className="bg-gray-700 p-4 rounded-2xl shadow-md my-2 flex items-center w-3/4 mx-auto">
    <ClockIcon className="w-5 h-5 mr-3 text-yellow-300" />
    <div>
      <p className="font-semibold text-white">{record.sensor}</p>
      <p className="text-sm text-gray-300">Value: {record.value} | {new Date(record.timestamp).toLocaleString()}</p>
    </div>
  </div>
);
...
function Alert() {
  ...
  return (
    <div className="min-h-screen bg-background-color p-6 flex flex-col items-center justify-center">
      <motion.h1 
        initial={{ opacity: 0 }} 
        animate={{ opacity: 1 }} 
        transition={{ duration: 1 }}
        className="text-4xl font-extrabold text-center mb-6 text-highlight-color"
      >
      ...
      <div className="w-full max-w-5xl">
        <section className="mb-8 text-center">
          <h2 className="text-2xl font-bold text-primary-color mb-3">📊 Real-Time Sensor Data</h2>
          <BarGraph data={temperatureData} label="Temperature" />
          <BarGraph data={vibrationData} label="Vibration" />
        </section>

        <section className="mb-6 text-center">
          <h2 className="text-2xl font-bold text-secondary-color mb-3">🔔 Real-Time Alerts</h2>
          {alerts.length === 0 ? (
        ...
        <section className="mt-6 text-center">
          <h2 className="text-2xl font-bold text-highlight-color mb-3">📋 Sensor Records</h2>
          {records.length === 0 ? (
